...
{
    // Use standard function if available or regex
...
if ($action === 'delete') {
    ...
    try {
        ...
        if ($page) {
            ...
            // A. Veritabanındaki Temp Dosyalarını Temizle (Güvenlik için opsiyonel ama iyi olur)
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['get_slug'])) {
    ...
    if ($page) {
        ...
        if (empty($editContent)) {
            ...
            // Check Theme (if still empty)
            if (empty($editContent)) {
                // We need ACTIVE_THEME here. 
                // Since theme-init.php might be complex, let's try to get it from DB settings or fall back.
