...
try {
    ...
    // Son 7 Günlük Kayıt İstatistikleri (Chart.js için)
    $stats = [];
    for ($i = 6; $i >= 0; $i--) {
        $date = date('Y-m-d', strtotime("-$i days"));
        $displayDate = date('d M', strtotime($date));
        $count = $db->prepare("SELECT COUNT(*) FROM users WHERE DATE(created_at) = ?");
        $count->execute([$date]);
        $stats[$displayDate] = (int)$count->fetchColumn();
    }
...
<div class="row g-4 mb-4">
    <div class="col-12">
        <div class="card" style="background: var(--card); box-shadow: var(--shadow); border-radius: var(--radius);">
            <div class="card-header py-3 d-flex justify-content-between align-items-center" style="border-bottom: 1px solid var(--border);">
                <h6 class="m-0 fw-bold">
                    <i class="fas fa-chart-area me-2 text-primary"></i>
                    <span lang="reg_stats">Yeni Kullanıcı Kayıtları (Son 7 Gün)</span>
                </h6>
            </div>
            <div class="card-body">
...
<!-- Son Aktiviteler -->
<div class="card mb-4" style="background: var(--card); box-shadow: var(--shadow); border-radius: var(--radius);">
                            ...
                            <?php foreach ($recent_logs as $log): ?>
                                <tr>
                                    ...
                                    <td data-label="Kullanıcı">
                                        <?= e(!empty($log['username']) ? $log['username'] : 'System') ?>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php endif; ?>
        ...
        <div class="card h-100" style="background: var(--card); box-shadow: var(--shadow); border-radius: var(--radius);">
            <div class="card-body">
                ...
                <div class="d-grid gap-2">
                    <a href="index.php?page=pages" class="btn btn-outline-primary btn-responsive text-start">
                        <i class="fas fa-plus me-2"></i>
                        <span lang="new_page">Yeni Sayfa</span>
                    </a>
                    <a href="index.php?page=users" class="btn btn-outline-success btn-responsive text-start">
                        <i class="fas fa-user-plus me-2"></i>
                        <span lang="new_user">Yeni Kullanıcı</span>
                    </a>
                    <a href="index.php?page=modules" class="btn btn-outline-warning btn-responsive text-start">
                        <i class="fas fa-download me-2"></i>
                        <span lang="install_module">Modül Yükle</span>
                    </a>
                    <a href="index.php?page=system" class="btn btn-outline-secondary btn-responsive text-start">
                        <i class="fas fa-history me-2"></i>
