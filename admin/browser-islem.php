...
function safePath($root, $path) {
    ...
    if (!$full || strpos($full, $root) !== 0) return false;
    return $full;
}
...
if(isset($_GET['zip'])){
    ...
    if($p){
        ...
        if ($zip->open($zipFile, ZipArchive::CREATE | ZipArchive::OVERWRITE) === TRUE) {
            ...
            $zip->close();
            if (ob_get_level()) ob_end_clean(); // Bozulmayı önlemek için tamponu temizle
...
if(isset($_GET['api'])){
    ...
    if(!$full || !is_dir($full)) exit(json_encode([]));
    
    $out = [];
    foreach(scandir($full) as $f){
        if($f === '.' || $f === '..') continue;
        if(!$SHOW_HIDDEN && str_starts_with($f, '.')) continue;
        $fp = "$full/$f";
        $out[] = [
            'name' => $f,
...
if(isset($_GET['read'])){
    ...
    if($p && !is_dir($p)) echo json_encode(['ok'=>true, 'content'=>file_get_contents($p)], JSON_UNESCAPED_UNICODE);
    exit;
}
...
if(isset($_GET['rename'])){ $o=safePath($ROOT,$_POST['old']); $n=basename($_POST['new']); if($o&&$n) rename($o,dirname($o).'/'.$n); exit; }
if(isset($_GET['mkdir'])){ $b=safePath($ROOT,$_POST['path']); $n=basename($_POST['name']); if($b&&$n) mkdir($b.'/'.$n); exit; }
if(isset($_GET['touch'])){ 
    $b=safePath($ROOT,$_POST['path']); $n=basename($_POST['name']); 
    if($b && $n) file_put_contents($b.'/'.$n, ''); 
    exit; 
}
if(isset($_GET['upload'])){
    $b=safePath($ROOT,$_POST['path']);
    if($b && isset($_FILES['f'])) move_uploaded_file($_FILES['f']['tmp_name'],$b.'/'.basename($_FILES['f']['name']));
    exit;
}