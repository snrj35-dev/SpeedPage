    ...
    <div class="ms-auto d-flex align-items-center gap-3">
        <a href="<?= BASE_URL ?>php/profile.php?id=<?= $_SESSION['user_id'] ?>" 
           class="d-flex align-items-center text-decoration-none me-3 transition-hover">
            ...
            <span class="fw-bold text d-none d-sm-inline"><?= htmlspecialchars($finalName) ?></span>
        </a>
...
<nav class="nav px-4 py-2 border-bottom">
    <a href="index.php?page=settings" class="nav-link"><i class="fas fa-file-alt"></i> <span lang="settings"></span></a>
    <a href="index.php?page=pages" class="nav-link"><i class="fas fa-file"></i> <span lang="pages"></span></a>
    <a href="index.php?page=menu" class="nav-link"><i class="fas fa-clipboard-list"></i> <span lang="menu_management"></span></a>
    <a href="index.php?page=modules" class="nav-link"><i class="fas fa-puzzle-piece"></i> <span lang="modules"></span></a>
    <?php if (!empty($is_admin) && $is_admin): ?>
        <a href="index.php?page=users" class="nav-link"><i class="fas fa-users"></i> <span lang="users"></span></a>
        <a href="index.php?page=dbpanel" class="nav-link"><i class="fas fa-database"></i> <span lang="database"></span></a>
        <a href="index.php?page=browser" class="nav-link"><i class="fas fa-folder-open"></i> <span lang="filebrowser"></span></a>
...
<?php
switch ($page) {
    case 'settings': require __DIR__ . "/settings-panel.php"; break;
    case 'pages':    require __DIR__ . "/page-panel.php"; break;
    case 'menu':     require __DIR__ . "/menu-panel.php"; break;
    case 'modules':  require __DIR__ . "/modules-panel.php"; break;
    case 'users':    if (!empty($is_admin) && $is_admin) require __DIR__ . "/user-panel.php"; break;
    case 'dbpanel':  if (!empty($is_admin) && $is_admin) require __DIR__ . "/veripanel-content.php"; break;
    case 'browser':  if (!empty($is_admin) && $is_admin) require __DIR__ . "/browser-panel.php"; break;
    case 'system': if (!empty($is_admin) && $is_admin) require __DIR__ . "/system-panel.php"; break;
    default: echo "<p>Sayfa bulunamadı.</p>";
}
?>
</div>
