...
if (!$user) die('<div class="alert alert-danger small text-center"><span lang="user_not_found"></span></div>');
...
<head>
    ...
    <title>Profil Düzenle - <?= htmlspecialchars($user['username']) ?></title>
    ...
    <div class="row justify-content-center">
        <div class="col-md-8 col-lg-6">
                    ...
                    <form method="POST">
                        <div class="text-center mb-4">
                            <div class="profile-icon-big text-primary">
                                <i class="fas <?= htmlspecialchars($user['avatar_url'] ?? 'fa-user') ?>"></i>
                            </div>
                        ...
                        ?>
                       <label class="form-label small text-uppercase fw-bold opacity-50 mb-3" lang="choose_profile_icon"></label>
                        <div class="row row-cols-4 row-cols-sm-6 g-2 mb-4 overflow-auto" style="max-height: 250px; padding: 10px; background: rgba(0,0,0,0.2); border-radius: 15px;">
                            <?php foreach($icons as $class => $name): ?>
                                <div class="col text-center">
                                    <input type="radio" class="btn-check" name="avatar_url" id="<?= $class ?>" value="<?= $class ?>" <?= ($user['avatar_url'] == $class) ? 'checked' : '' ?>>
                                    <label class="btn btn-outline-primary border-0 w-100 p-3 rounded-3 shadow-sm" for="<?= $class ?>" title="<?= $name ?>">
                                        <i class="fas <?= $class ?> fa-xl d-block mb-1"></i>
                                        <small style="font-size: 0.6rem; display: block;margin-top: 10px;" class="text-truncate"><?= $name ?></small>
                                    </label>
                                </div>
                            <?php endforeach; ?>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="display_name" class="form-control bg-dark text-white border-0" value="<?= htmlspecialchars($user['display_name']) ?>" required>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="username" class="form-control bg-dark text-white border-0" 
                                   ...
                                   <?= ($allow_username === '1' || $is_admin) ? '' : 'readonly opacity-50' ?>>
                            <?php if($allow_username === '0' && !$is_admin): ?>
                                <small class="text-warning" style="font-size: 0.7rem;"><span lang="username_change_disabled"></span></small>
                            <?php endif; ?>
                        </div>
                        <?php if (($allow_password === '1' || $is_admin)): ?>
                        <div class="p-3 bg-dark bg-opacity-50 rounded-3 mb-4 border border-secondary border-opacity-25 mt-4">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="password" name="current_password" class="form-control bg-dark text-white border-secondary border-opacity-25" 
                            ...
                            <div class="mb-2">
                                ...
                                <input type="password" name="new_password" class="form-control bg-dark text-white border-secondary border-opacity-25" 
                                    data-placeholder="new_password_placeholder">
                                <small class="text-muted" style="font-size:0.65rem;"><span lang="only_fill_if_want"></span></small>
                            </div>
                        </div>
                        <?php else: ?>
            ...
            <div class="text-center mt-3">
                <a href="../index.php" class="text-white-50 text-decoration-none small"><i class="fa-solid fa-arrow-left"></i> <span lang="return_home"></span></a>
            </div>
